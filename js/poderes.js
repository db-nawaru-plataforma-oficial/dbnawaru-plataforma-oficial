                        ...
                        ${poder.imagenes.slice(0, 4).map((img, idx) => `
                            <div class="content-card-image">
                                ...
                                ${poder.imagenes.length > 4 && idx === 3 ? 
                                    `<div style="position:absolute;inset:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;color:white;font-size:1.5rem;font-weight:bold;">+${poder.imagenes.length - 4}</div>` 
                                : ''}
                            </div>
                        `).join('')}
                    </div>
    ...
    try {
        if (imagenesFiles.length > 0) {
            for (const file of imagenesFiles) {
                const fileName = `poderes/${Date.now()}_${Math.random().toString(36).substr(2, 9)}_${file.name}`;
...
async function editItem(id) {
    try {
        ...
        console.error('Error:', error);
        alert('❌ Error al cargar el poder elemental');
    }
}
